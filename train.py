...
def build_scheduler(optimizer, warmup_epochs: int, T_0: int, T_mult: int):
    ...
    cosine = optim.lr_scheduler.CosineAnnealingWarmRestarts(
        ...
        T_0=T_0,
        T_mult=T_mult,
        eta_min=1e-7,       # slightly lower floor than original — we're near convergence
    )
...
def train():
    ...
    checkpoint_path  = base_path / PRETRAINED_MODEL_PATH
    ...
    try:
        ...
        print(
            f"\n[ERROR] load_state_dict failed.\n"
            f"  This means the architecture defined here does not match the one\n"
            f"  used when the checkpoint was saved.  Check that ResNet18Classifier\n"
            f"  is byte-for-byte identical to the class in train_improved.py.\n"
            f"  Details: {e}"
        )
        return 1
    ...
    # Quick sanity-check: log how many parameters were loaded
    n_params = sum(p.numel() for p in model.parameters())
    print(f"  [OK] Model has {n_params:,} parameters.")
    ...
    metric_schemas = {
        ...
        "predicted": tlc.CategoricalLabelSchema(display_name="predicted label",class_names=["chihuahua","muffin"]),
